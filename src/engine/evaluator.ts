...
/** Родительный падеж — для «стрит до туза», «флеш до короля». */
const OF: Record<number, string> = {
...
/** Родительный множественного — «пара тузов», «сет девяток», «каре королей». */
...
function titleFor(category: HandCategory, r: number[]): string {
  ...
  switch (category) {
    ...
    case TRIPS: return `Сет ${ofMany(r[0])}`
