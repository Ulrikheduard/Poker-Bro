import { type Card, RANK_GENITIVE } from './cards'
...
/** Родительный падеж — для «стрит до туза», «флеш до короля». Таблица одна
 *  на движок и на экраны: две копии однажды разъехались бы. */
const OF = RANK_GENITIVE
/** Именительный множественного — «две пары, тузы и короли». */
...
/** Родительный множественного — «пара тузов», «тройка девяток», «каре королей». */
const OF_MANY: Record<number, string> = {
...
/** Предложный множественного — «фулл-хаус, тузы на королях». */
const ON_MANY: Record<number, string> = {
  2: 'двойках', 3: 'тройках', 4: 'четвёрках', 5: 'пятёрках', 6: 'шестёрках', 7: 'семёрках',
  8: 'восьмёрках', 9: 'девятках', 10: 'десятках', 11: 'валетах', 12: 'дамах', 13: 'королях', 14: 'тузах',
}
...
function titleFor(category: HandCategory, r: number[]): string {
  ...
  const of = (x: number) => OF[x] ?? String(x)
  const many = (x: number) => MANY[x] ?? String(x)
  const ofMany = (x: number) => OF_MANY[x] ?? String(x)
  const onMany = (x: number) => ON_MANY[x] ?? String(x)
  switch (category) {
    case STRAIGHT_FLUSH: return r[0] === 14 ? 'Флеш-рояль' : `Стрит-флеш до ${of(r[0])}`
    case QUADS: return `Каре ${ofMany(r[0])}`
    case FULL_HOUSE: return `Фулл-хаус, ${many(r[0])} на ${onMany(r[1])}`
    case FLUSH: return `Флеш до ${of(r[0])}`
    case STRAIGHT: return `Стрит до ${of(r[0])}`
    // Не «сет»: сетом тройку называют только тогда, когда пара была
    // на руках, а оценщик видит семь карт вперемешку и знать этого не может.
    // Раздел «Комбинации» называет её тройкой — значит, и здесь тройка.
    case TRIPS: return `Тройка ${ofMany(r[0])}`
    case TWO_PAIR: return `Две пары, ${many(r[0])} и ${many(r[1])}`
    case PAIR: return `Пара ${ofMany(r[0])}`
